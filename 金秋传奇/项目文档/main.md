

## 一些物料

微软账号
zhuxiaohang2012@hotmail.com
zhums-001

google账号
XiuZZZZZ
zhuxiaohang2012@hotmail.com
zhugh-001

## 如何开始项目

### 环境准备

- git 准备
  - 下载，安装
  - git config --global user.name "XiuZZZZZ"
  - git config --global user.email "zhuxiaohang2012@hotmail.com"
- idea：下载idea，导入easyclick插件
- idea：设置-tab缩进：设置-按键-缩进-设置tab
- opencode：下载opencode， 添加模型，选择项目 
- 
### 打开项目

- 新项目
  - github创建仓库，克隆这个项目 https://github.com/XiuZZZZZ/jinqiuchuanqi.git
- 老项目
  - 直接克隆项目即可 https://github.com/XiuZZZZZ/jinqiuchuanqi.git

## 工作流

### comment 提交到远程仓库


## 其他

### 进阶使用 ai 赋能
- ima https://ima.qq.com/wiki/?shareId=e215ccd95d5440a4b1fa97339fbc0aaef07a041459ab2ee7781163f2039e1253&webFrom=10000079
- 官方入口: https://ieasyclick.com/


### 我的easyclick账户


### 商业化
- 购买卡密
    - http://laoleng.vip/docs/tools/easyclick/ios-qk/qa/authorize/
-